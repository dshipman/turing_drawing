...
- **Speed** (global) — how hard the simulation runs each frame (`0` = paused, `1` = max, step `0.001`; default `0.5`). This is a fraction of **Max itrs/frame**. Type a value beside the slider to set it exactly. In **Normalised** machine schedule mode, this budget is a fixed pool of machine-steps per frame rather than independent per-machine rates.
...
- **Normalised** — toggle at the top of the machines list. On (default), active machines share one step of budget per round in proportion to their speed weights. Off keeps absolute rates: each machine steps independently at `10^(speed / 10)` per round. The choice is remembered in Settings and is not saved in presets.
